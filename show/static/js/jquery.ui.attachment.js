	...
	$.widget('ui.attachment', {
		...
		_upload : function (file){
			...
			if(!this.frame && !frame.length){
				...
				.css({left : '-1700px', position : 'absolute'}).hide().appendTo('body');
			}else{
				this.frame = frame;
			}
			// 上传文件
			...
			(function (attachmentInst){
				...
				$.ajax({
					...
					success : function (data){
					    ...
					    attachmentInst._trigger('afterRemove',e,data);
						loadTip.remove();
		...
		_create : function() {
			...
			$('<input/>',{type : 'button',value : '文件上传'})
				.addClass('ui-attchment-button ui_btn ui_btn_small ui_btn_info')
				.addClass('submitLong')
